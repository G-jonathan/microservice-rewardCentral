package com.openclassroomsproject.microservicerewardCentral.utils;
...
public class CustomRequestLoggingFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
        ...
        LOGGER.info("#[MS-RewardCentral]-> Call - CONTROLLER: Request URL: {}", requestURL);
        try {
            chain.doFilter(currentRequest, servletResponse);
        } finally {
            int status = currentResponse.getStatus();
            LOGGER.info("#[MS-RewardCentral]-> Response status: {}", status);
        }
    }
}